...
| 4 | Enable per-separator MIP profiling (uncomment implbound/clique/tableau/path/mod-k clocks) | `highs/mip/HighsSeparation.cpp` `highs/mip/HighsSeparator.cpp` `highs/mip/MipTimer.h:164-174,316` | 1 | done (1.15.1.4) | Wired `kMipClockImplbound/Clique/Tableau/Path/ModK` via `MipTimer` + conditional `profiling->mip_` guard. Super-fast 1.10x (5 faster/13 slower) overhead ~8% when profiling enabled; 0% when off. Enables Tier2 cut measurement. |
