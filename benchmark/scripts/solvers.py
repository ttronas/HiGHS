...
from common import results_dir, run_capture, sha256_file  # noqa: F401
...
# ------------------------------------------------------------------------
# The ground truth for correctness checks is a solver whose results are
# treated as reference (currently Gurobi, licensed via benchmark/gurobi.lic).
# Its per-instance records live in the committed cache:
#     results/{gt_solver}/{gt_version}/{machine}/{set}/{instance}.json
# Adding another ground-truth provider = implement it as a Solver above and
# register it here; compare_versions.py picks it up automatically.

GROUND_TRUTH_PROVIDERS: dict[str, type[Solver]] = {
    "gurobi": GurobiSolver,
}


def ground_truth_version(solver: str = "gurobi",
                         results_root: Path | None = None) -> str | None:
    """Latest cached version of the ground-truth solver, or None.

    Versions are compared as dotted integers so 12.0.3 sorts after 12.0.
    """
    if solver not in GROUND_TRUTH_PROVIDERS:
        raise SystemExit(
            f"Unknown ground-truth provider '{solver}'. "
            f"Known: {', '.join(GROUND_TRUTH_PROVIDERS)}")
    root = results_root if results_root is not None else results_dir()
    g_dir = root / solver
    if not g_dir.is_dir():
        return None
    ...
    vers = [p.name for p in g_dir.iterdir() if p.is_dir()]
