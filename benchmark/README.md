...
# smoke set: the bundled examples (gate before any set run)
uv run python scripts/run_benchmark.py --instances-root examples --set smoke-test
...
For version comparisons use the modular comparator:
...
`saved_s`, shifted geomeans) and RELATIVE values (delta %, speedup,
shifted-geomean ratio, faster/slower/equal counts). Exit codes: `0` ok,
`1` ground-truth mismatch (INVALID SIGNAL — blocks promotion), `2` missing data.
